...
async function getSettings() {
	return await instance
		.get(get(Api)['users.settings'] + get(User).name)
		.then((res) => {
			return res.data
		})
}
...
async function setSettings() {
	...
	const resp = await instance
		.post(get(Api)['users.settings'] + get(User).name, formdata, {
	...
	if (decoded != false) {
		...
		User.set({
			...
			theme_mode: get(User).theme_mode,
			auth: true,
		})
	} else {
		//alert
...
export { getSettings, setSettings, settings_modified }
