<script>
	...
	function goBack() {
		...
		if (directElement.style.transform) directElement.style.transform = ''
	...
	onMount(async () => {
		...
		_document = document
		document.addEventListener('changedChat', async () => {
	...
	onDestroy(async () => {
		if (_document) {
			_document.removeEventListener('changedChat', async () => {
				messages = await loadChat()
			})
		}
	})
		...
		<div class="header">
			<div class="user">
				<i
					...
					on:click={goBack}
					style="margin-right: 1rem;font-size: 1.5rem;top: 0;bottom: 0;margin:
					auto 0.5rem;color: var(--color);" />
				<div class="img">
					<img
						data={$currentChat.members[0].avatar}
						alt="profile image"
						height="40px"
						width="40px" />
				</div>
				<div class="info">
					<span style="font-size: 1.1rem;font-weight: 500;">
						{$currentChat.members[0].real_name}
					</span>
				...
				{#each messages as message}
					{#if message.new_day == true || message.new_day_from_last == true}
						<div class="date-splitter" style="display: flex">
							<hr />
							<span style="width: calc({message.on.length} * 0.8rem)">
								{message.on}
							</span>
							<hr />
						</div>
		...
		<div class="message">
			<div class="textarea">
				<textarea
					...
					name="message"
					id="message"
					style="margin-bottom: 0;min-height: 1.2rem;"
					contenteditable />
			</div>
			<button
				on:click={() => {
