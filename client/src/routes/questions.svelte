<script context="module">
    ...
    export async function preload(page,session){
        let isSSRPage;
        const res = instance.get(get(Api)['home.index']+'?mode=questions');
...
currentApi.set({data: instance.get(get(Api)['home.index']+'?mode=questions')});

export let data;
</script> 

<svelte:head>
<title>NewApp - Questions</title>
