<script>
	...
	async function CheckUsername() {
		const res = await instance
			.get($Api['auth.check_username'] + username, { progress: false })
	...
	async function CheckEmail() {
		const res = await instance
			.get($Api['auth.check_email'] + email, { progress: false })
			.then(function (response) {
				return response.data
			})
		const json = await res
		if (json.check) {
			...
			/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{1,4})+$/.test(email) === false
		) {
	...
	async function RegisterUser() {
		...
		register_check = await Auth.register({
			username: username,
			email: email,
		...
		{#if register_check == ''}
			...
			<div class="modal-footer">
				<span style="font-size: 0.6rem;color:var(--color);">
					...
					<span style="color:#18BC9C;cursor:pointer;" on:click={OpenLogin}>
						Login here
					</span>
				</span>
				<button type="submit" class="modal-button" on:click={RegisterUser}>
					Register
				</button>
			</div>
		{:else if register_check == 'success'}
			<div class="modal-content">
				...
				<p class="lead">
					We have sent you an email for confirm your action on this website, got
					to check your mail box for continue your action.
				</p>
			</div>
			<div class="modal-footer">
				<button
